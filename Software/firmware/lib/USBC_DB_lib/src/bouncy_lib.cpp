...
* DEV: GitHub@Rr42
* Description:
*  Library based on bouncy.cpp for the 8x10 RGB LED display that
*   shows an animated ball bouncing around in a white box.
*   SEIZURE WARNING!
******************************************************************/
...
const uint16_t bouncy_start[COLOR_COUNT][LINE_COUNT] = {
                                                 { 0b1111111111,
                                                   0b1000000001,
                                                   0b1000000001,
                                                   0b1000000001,
...
/* Frame buffer */
uint16_t bouncy_buffer[COLOR_COUNT][LINE_COUNT] = {
                                            { 0b0000000000,
                                              0b0000000000,
                                              0b0000000000,
                                              0b0000000000,
...
void move_ball(bool reflect) {
    ...
    if (!reflect){
        bouncy_buffer[RED][ball.y_pos] &= ( ( ~(0x1<<ball.x_pos) )|0b10000000001 );
        bouncy_buffer[GREEN][ball.y_pos] &= ( ( ~(0x1<<ball.x_pos) )|0b10000000001 );
        bouncy_buffer[BLUE][ball.y_pos] &= ( ( ~(0x1<<ball.x_pos) )|0b10000000001 );
    }
    ...
    {
        case 1: // 001
            bouncy_buffer[RED][ball.y_pos]   |= 0x0;
            bouncy_buffer[GREEN][ball.y_pos] |= 0x0;
            bouncy_buffer[BLUE][ball.y_pos]  |= (0x1<<ball.x_pos);
            break;
        case 2: // 010
            bouncy_buffer[RED][ball.y_pos]   |= 0x0;
            bouncy_buffer[GREEN][ball.y_pos] |= (0x1<<ball.x_pos);
            bouncy_buffer[BLUE][ball.y_pos]  |= 0x0;
            break;
        case 3: // 011
            bouncy_buffer[RED][ball.y_pos]   |= 0x0;
            bouncy_buffer[GREEN][ball.y_pos] |= (0x1<<ball.x_pos);
            ...
            break;
        case 4: // 100
            bouncy_buffer[RED][ball.y_pos]   |= (0x1<<ball.x_pos);
            bouncy_buffer[GREEN][ball.y_pos] |= 0x0;
            bouncy_buffer[BLUE][ball.y_pos]  |= 0x0;
            break;
        case 5: // 101
...
void init_bouncy(){
    ...
    for (uint8_t i=0; i<LINE_COUNT; ++i) {
        bouncy_buffer[RED][i]   = bouncy_start[RED][i];
        bouncy_buffer[GREEN][i] = bouncy_start[GREEN][i];
        bouncy_buffer[BLUE][i]  = bouncy_start[BLUE][i];
    }
}
